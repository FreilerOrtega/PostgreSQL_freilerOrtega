...
-- cambiar el nombre de una columna dentro de una tabla 
alter table full_order_info  rename column firts_name to   first_name ;

--para cambiar el tipo de dato 
alter table full_order_info alter column stockmin type integer;


-- sacar el total de dinero invertido en un stock 
select sum (stock_price ) as stock_price , sum(stock) as stock from full_order_info group by name ;


-- se saca el total y lo ordena por el nombre de el producto 
...
--llamar vista ya creada 

select * from stock_product where name='webcam' ;


-- vista materializada permite ver los productos antes de ser insertado algo nuevo 
-- despues que se refresca si aparecera lo nuevo 
create materialized stock_avg as
select sum(stock_price) as stock_price,sum(stock) as stock, stockmin, stockmax, name from full_order_info group by name, stockmin, stockmax;
...
-- otro tipo de join trayendo cosas especificas 
create view order_details_view as 
select users.id, users.first_name, orders.orderdate,od.quantity,od.price,p.name
from users
...
where users_id = p_id_user;

 raise notice 'el total de dinero gastado % es %',p_id_user,total;
end;
$$;
...
VALUES (
    13, 10, 5, 428.26,
    '2025-03-31', '5', 'Christopher', 'Charles',
    'christopher.charles@yahoo.com', '8.132.55.99', 'https://barrera.com/',
    'USB Drive', 'Ahead he cup fine course exactly.', 89,
    428.26, 45, 172
);
